...
class Operation(AbstractSlackEscapeOperation):
    ...
    def execute_task(self, args):
        ...
        dm_path = self.get_slack_export_root().joinpath('multiuser_direct_messages')
        if not dm_path.exists():
            dm_path.mkdir()

        for channel in total_channels:
            channel_root = dm_path.joinpath(channel['name'])
    ...
    def load_all_channels(self, args):
        ...
        cursor = None
        while True:
            response = self.get_slack_web_client(args).conversations_list(types='mpim', cursor=cursor, limit=args.limit)
